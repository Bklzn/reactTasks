    ...
    <>
      <div className="max-w-full border-2 border-gray-500 p-4 rounded-lg text-white">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col">
          ...
          {simpleError && <p className="text-red-500 text-sm">{simpleError}</p>}
        </form>
      </div>
      <div className="border-2 border-gray-500 p-4 rounded-lg text-white">
        <label className="text-sm pb-2">
          Enter your Go REST Token here. It's only saved in your browser in
          local storage and used by this app
        </label>
        <input
          type="text"
          autoComplete="off"
          className="w-full bg-neutral-500 rounded p-1 px-2 m-auto"
          placeholder="Enter Go REST Token here"
          value={token}
        ...
        <Button
          type="button"
          onClick={saveToken}
          className="mt-2 max-w-[100px] block mt-auto"
        >
          Submit
        </Button>
      </div>
